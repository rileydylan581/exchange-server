...
const getAccount = async (req, res) => {
  ...
  await config.Account.findOne({ aid: aid })
    .exec()
    .catch((err) => {
      return res
        .status(500)
    ...
    .then((result) => {
      if (!result) {
        return res.status(404).json({
          message:
            "Could Not Find Account. This Could Be Because It Doesn't Exist Or Was Deleted.",
...
const deleteAccount = async (req, res) => {
  ...
  await config.Account.deleteOne({ aid: aid })
    .exec()
    .catch((err) => {
      return res
        .status(400)
